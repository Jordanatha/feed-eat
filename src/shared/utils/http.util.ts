...
export namespace HttpUtil {
    ...
    export interface QuerySerializeable {
        fields?: {
    ...
    export function serializeQuery(queries: HttpUtil.QuerySerializeable): any {
        const data: any = {};

        for (const key in queries) {
            for (const resource in queries[key]) {
                data[`${key}[${resource}]`] = queries[key][resource].join(',');
            }
        }
